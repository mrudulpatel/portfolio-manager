...
import { collection, addDoc, onSnapshot, getDocs, deleteDoc, updateDoc } from "firebase/firestore";
...
const Dashboard = () => {
		...
		const fetchData = async () => {
			...
			onSnapshot(stocksCollection, async (snap) => {
				const promises = snap.docs.map(async (doc) => {
					const res = await fetch(`https://priceapi.moneycontrol.com/pricefeed/nse/equitycash/${doc.data().symbol}`, {
						method: "GET",
						headers: {
				...
				setStocks(filteredStocksData.sort((a, b) => {
	...
	return (
		<main className="min-h-screen">
			{/* Navbar */}
			<nav className="flex sticky justify-between items-center h-16 text-white  shadow-sm" role="navigation">
				<div className="flex flex-row items-center">
					<a href="/" className="pl-8 text-lg text-black dark:text-white">Portfolio Manager</a>
					<h1 className="text-lg ml-3 text-gray-800 dark:text-gray-400">ncpatel25@gmail.com</h1>
							...
							<Dialog.Content className="data-[state=open]:animate-contentShow fixed top-[50%] left-[50%] max-h-[85vh] w-[90vw] max-w-[450px] translate-x-[-50%] translate-y-[-50%] rounded-[6px] bg-white text-black p-[25px]  focus:outline-none">
								...
								<form onSubmit={handleAddStock}>
									<fieldset className="mb-[15px] flex items-center gap-5">
										<label className="w-[140px] text-left text-[15px]" htmlFor="Stock Symbol">
									...
									<fieldset className="mb-[15px] flex items-center gap-5">
										<label className="w-[140px] text-left text-[15px]" htmlFor="Purchase Quantity">
											Purchase Quantity
										</label>
										<input
											className="border inline-flex h-[35px] w-full flex-1 items-center justify-center rounded-[4px] px-[10px] text-[15px] leading-none  outline-none "
									...
									<fieldset className="mb-[15px] flex items-center gap-5">
										<label className="w-[140px] text-left text-[15px]" htmlFor="Buy Price">
											Buy Price
										</label>
										<input
											className="border inline-flex h-[35px] w-full flex-1 items-center justify-center rounded-[4px] px-[10px] text-[15px] leading-none  outline-none "
									...
									<div className="mt-[25px] flex justify-end">
										<Dialog.Close asChild id="add_dialog_close" onClick={() => {
											setSymbol("");
											setQuantity(0);
											setBuy(0);
										}}>
											<button
												className="bg-red-400 transition duration-100 ease-in-out hover:bg-red-500  inline-flex h-[35px] items-center justify-center rounded-[4px] px-[15px] font-medium leading-none  focus:outline-none">
												Cancel
											</button>
										</Dialog.Close>
										<button
			...
			<section className="flex flex-col max-h-[calc(100vh - 4rem)] items-center justify-center overflow-hidden">
				<TableContainer sx={{ maxHeight: "calc(100vh - 4rem) !important" }}>
					<Table stickyHeader sx={{ textAlign: "center !important" }} className="w-full border overflow-auto">
						<TableHead sx={{ textAlign: "center !important" }}>
							<TableRow sx={{ textAlign: "center !important" }}>
								<TableCell sx={{ background: "black", color: "white !important", textAlign: "center " }} className="border w-[fit-content]">Sr. No.</TableCell>
								<TableCell sx={{ background: "black", color: "white !important", textAlign: "center " }} className="border">Stock Name</TableCell>
								<TableCell sx={{ background: "black", color: "white !important", textAlign: "center " }} className="border">Current Price</TableCell>
								<TableCell sx={{ background: "black", color: "white !important", textAlign: "center " }} className="border">52 Week High</TableCell>
								<TableCell sx={{ background: "black", color: "white !important", textAlign: "center " }} className="border">52 Week Low</TableCell>
								<TableCell sx={{ background: "black", color: "white !important", textAlign: "center " }} className="border">Quantity</TableCell>
								<TableCell sx={{ background: "black", color: "white !important", textAlign: "center " }} className="border">Buy Price</TableCell>
								<TableCell sx={{ background: "black", color: "white !important", textAlign: "center " }} className="border">% Less from high</TableCell>
								<TableCell sx={{ background: "black", color: "white !important", textAlign: "center " }} className="border">Latest Value</TableCell>
								<TableCell sx={{ background: "black", color: "white !important", textAlign: "center " }} className="border">Overall Gain/Loss %</TableCell>
								<TableCell sx={{ background: "black", color: "white !important", textAlign: "center" }} className="border">Overall Gain/Loss (in ₹)</TableCell>
								<TableCell sx={{ background: "black", color: "white !important", textAlign: "center " }} className="border">Actions</TableCell>
							</TableRow>
						</TableHead>
						...
						<TableBody sx={{ textAlign: "center !important" }}>
							{stocks.length === 0 ? (
								<TableRow>
									<TableCell className="text-white text-center" colSpan="11">No Stocks Added</TableCell>
								</TableRow>
							) : (
								stocks.map((stock, index) => (
									<TableRow
										key={index}
										className={index % 2 === 0 ? "bg-blue-300 text-black" : "bg-blue-200 text-black"}
									>
										<TableCell sx={{ textAlign: "center", fontSize: '15px' }} className="border">{index + 1}</TableCell>
										<TableCell sx={{ textAlign: "center", fontSize: '15px' }} className="border">{stock.companyName}</TableCell>
										<TableCell sx={{ textAlign: "center", fontSize: '15px', fontWeight: "bold" }} className="border">{stock.price}</TableCell>
										<TableCell sx={{ textAlign: "center", fontSize: '15px' }} className="border">{stock.high}</TableCell>
										<TableCell sx={{ textAlign: "center", fontSize: '15px' }} className="border">{stock.low}</TableCell>
										<TableCell sx={{ textAlign: "center", fontSize: '15px' }} className="border">{stock.quantity}</TableCell>
										<TableCell sx={{ textAlign: "center", fontSize: '15px' }} className="border">{stock.buy.toFixed(2)}</TableCell>
										<TableCell sx={{ textAlign: "center", fontSize: '15px', fontWeight: "bold" }} className="border">
											{((stock.high - stock.price) / stock.high * 100).toFixed(2)}
										</TableCell>
										<TableCell sx={{ textAlign: "center", fontSize: '15px' }} className="border">{(stock.price * stock.quantity).toFixed(2)}</TableCell>
										<TableCell sx={{ textAlign: "center", fontSize: '15px' }} className="border">
											{(((stock.price - stock.buy) / stock.buy) * 100).toFixed(2)}
										</TableCell>
										<TableCell sx={{ textAlign: "center", fontSize: '15px' }} className="border">
												...
												<Dialog.Portal>
													...
													<Dialog.Content className="data-[state=open]:animate-contentShow fixed top-[50%] left-[50%] max-h-[85vh] w-[90vw] max-w-[450px] translate-x-[-50%] translate-y-[-50%] rounded-[6px] bg-white text-black p-[25px] focus:outline-none">
														...
														<form onSubmit={handleEditStock}>
															...
															<div className="mt-[25px] flex justify-end">
																<Dialog.Close asChild id="dialog_close" onClick={() => {
																	setSymbol("");
																	setQuantity(0);
																	setBuy(0);
																}}>
																	<button
																		className="bg-red-500 hover:bg-red-600  inline-flex h-[35px] transition duration-100 ease-in-out items-center justify-center rounded-[4px] px-[15px] font-medium leading-none  focus:outline-none">
																		Cancel
																	</button>
																</Dialog.Close>
																<button
